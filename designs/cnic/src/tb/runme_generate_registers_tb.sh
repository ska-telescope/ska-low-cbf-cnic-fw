...
python3 ../../../../../../tools/args/gen_tb_config.py -c trafgen.ccfg -i registers.txt -o registers_tb.txt
exit
...
PRJ_DIR=$GITREPO/build/alveo/vivado/$PERSONALITY/ 
popd


if [ "$1" = "" ]; then
    echo -e "Assuming latest FPGA build directory to source ${PERSONALITY}.ccfg"
    cd $GITREPO/build/alveo/vivado/$PERSONALITY
...
python3 $GITREPO/tools/args/gen_tb_config.py -c ${PERSONALITY}.ccfg -i registers.txt -o registers_tb.txt

echo -e "Copy registers_tb.txt into the fpga build directory $PRJ_DIR where the file is referenced from the testbench script"
cp registers_tb.txt $PRJ_DIR
